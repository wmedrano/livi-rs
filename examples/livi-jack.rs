...
fn main() {
    ...
    let plugin = livi
        .iter_plugins()
        .find(|p| p.uri() == "http://drobilla.net/plugins/mda/JX10")
        .unwrap();
    ...
    let inputs: Vec<jack::Port<jack::AudioIn>> = plugin
        ...
        .inspect(info!("Initializing audio input {}.", p.name))
    ...
    let mut outputs: Vec<jack::Port<jack::AudioOut>> = plugin
        ...
        .inspect(info!("Initializing audio output {}.", p.name))
    ...
    let controls: Vec<f32> = plugin
        ...
        .collect();
    let mut events = livi::midi::Lv2AtomSequence::new(4096);
